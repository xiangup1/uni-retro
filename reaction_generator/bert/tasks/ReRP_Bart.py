...
@register_task("ReRP_Bart")
class ReRPBartTask(ReRPTask):
    """Task for training transformer auto-encoder models."""

    @staticmethod
    def add_args(parser):
        """Add task-specific arguments to the parser."""
        ReRPBartTask.add_other_args(parser)
        ReRPBartTask.add_default_encoder_args(parser)
    ...
    def add_other_args(parser):
        ...
        parser.add_argument(
            "--infer-task-type",
            default='retrosynthetic',
            choices=['retrosynthetic', 'synthetic', 'value_fn'],    # RMK: modified here!
...
@register_task("ReRP_Bart_RS")
class ReRPBartTask_RS(ReRPBartTask):
    def __init__(self, args, dictionary):
        super().__init__(args, dictionary)
        self.task_type = "retrosynthetic"


@register_task("ReRP_Bart_S")
class ReRPBartTask_S(ReRPBartTask):
    def __init__(self, args, dictionary):
        super().__init__(args, dictionary)
        self.task_type = "synthetic"


@register_task("ReRP_Bart_VF")
class ReRPBartTask_VF(ReRPBartTask):
    def __init__(self, args, dictionary):
        super().__init__(args, dictionary)
        self.task_type = "value_fn"

    def one_dataset(self, raw_dataset, coord_seed, mask_seed, **kwargs):
        input_name, output_name = 'smiles', 'value'
        
        token_dataset = KeyDataset(raw_dataset, input_name)
        val_dataset = KeyDataset(raw_dataset, output_name)
        ...
        token_dataset, val_dataset = map_data_align_check_aug_dataset(token_dataset, val_dataset)
        # RMK: add front and end token
        token_dataset = PrependAndAppend(token_dataset, self.dictionary.bos(), self.dictionary.eos()) 
        return {
            "src_tokens": RightPadDataset(
                token_dataset,
            ...
            "value_fn": RawLabelDataset(val_dataset),
        }, {
    ...
    def one_dataset_inference(self, raw_dataset, **kwargs):
        ...
        return {
            ...
            "value_fn": RawLabelDataset(val_dataset),
            # "reaction_type": RawLabelDataset(val_dataset),
        }, {
        ...
        token_dataset = map_data_aug_dataset(token_dataset)
        # out_dataset = map_data_aug_dataset(out_dataset)
        token_dataset = PrependAndAppend(token_dataset, self.dictionary.bos(), self.dictionary.eos()) 
        # out_dataset = PrependAndAppend(out_dataset, self.dictionary.bos(), self.dictionary.eos())
        return {
            "src_tokens": RightPadDataset(
                token_dataset,
                pad_idx=self.dictionary.pad(),
    ...
    def infer_step(self, args, sample, generator, single_sample=True, **kwargs):
        src_tokens = sample['net_input']['src_tokens']      
        pre_src_tokens = sample['net_input']['pre_src_tokens']    
        pred = generator(src_tokens, pre_src_tokens)
        # pred = generator(sample)
        # print(src_tokens, pre_src_tokens)
        pred_val = pred[0].flatten().tolist()
        if single_sample:
            pred_val = pred_val[0]
        return pred_val