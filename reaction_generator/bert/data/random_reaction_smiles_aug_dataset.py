...
class RandomReactionSmilesAugDataset(BaseWrapperDataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.times):
            ...
            pro_smi_l.append(pro_smi)
            rec_smi_l.append(rec_smi)
            pro_map_num_smi_l.append(pro_map_num_smi)
            reactant_map_num_smi_l.append(reactant_map_num_smi)
        return pro_smi_l, rec_smi_l, pro_map_num_smi_l, reactant_map_num_smi_l 
    ...
    def clear_map_canonical_smiles(self, smi, canonical=True, root=-1):
        ...
        # except:
        #     smi = sorted(smi.split('.'), key = lambda x: len(x), reverse = True)[0] 
        #     mol = Chem.MolFromSmiles(smi)
        #     if mol is not None:
        #         for atom in mol.GetAtoms():
        #             if atom.HasProp('molAtomMapNumber'):
