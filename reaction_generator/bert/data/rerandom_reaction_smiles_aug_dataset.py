...
from .data_utils import smi_tokenizer, canonical_smiles_with_am, remove_am_without_canonical, canonical_smiles, rooted_smiles_with_am


class ReRandomReactionSmilesAugDataset(BaseWrapperDataset):
    ...
    def get_rooted_alignment_smiles(self, product, reactant):
        ...
        if cano_atom_map is None:
            # 返回未加处理的 smile
            return remove_am_without_canonical(product), ".".join([remove_am_without_canonical(i) for i in reactant]), product,  reactant
    ...
    def clear_map_canonical_smiles(self, smi, canonical=True, root=-1):
        ...
        aug_am_smi = rooted_smiles_with_am(smi,  root = root, canonical=canonical)
        aug_smi = remove_am_without_canonical(aug_am_smi)
