...
@register_model("reaction_unit")
class ReactionUnitModel(BaseUnicoreModel):
    @staticmethod
    def add_args(parser):
    ...
    def forward(
        ...
        **kwargs
        ...
        if not padding_mask.any():
            padding_mask = None
        x = self.embed_tokens(reverse_src_tokens)
        ...
        # print('test src_tokens: ', src_tokens, reverse_src_tokens)
        ...
        return classifier_logits, decoder_outprob, vq1, vq2
    ...
    def forward_decoder(
        ...
        **kwargs
        ...
        if self.use_decoder:          
            ...
            vq1 = None
            if self.use_vq_embedding:
                vq1 = self.attention_head_tar(encoder_cls, self.vq_embeddings)
                # encoder_vq = self.gumbel_softmax(encoder_rep, self.vq_embeddings)
                encoder_vq = vq1
                encoder_cls[:,0,:] = encoder_vq

            decoder_rep = self.decoder(x_decoder, padding_mask=decoder_padding_mask, encoder_padding_mask=encoder_padding_mask, encoder_out=encoder_cls, attn_mask=None)
            decoder_outprob = self.decoder_lm_head(decoder_rep)           
...
@register_model_architecture("reaction_unit", "reaction_unit")
...
@register_model_architecture("reaction_unit", "reaction_unit_base")
