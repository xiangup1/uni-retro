...
from ..modules import init_bert_params
...
class BARTModel(TransformerModel):
    ...
    def forward_decoder(
        ...
        if want_probs:
            probs = self.get_normalized_probs(
                x, log_probs=True, temperature = temperature, sample=None
            )
            probs = probs[:, -1, :]
            return probs, None
    ...
    def get_normalized_probs(self, net_output, log_probs, temperature = 1.0, sample=None):
    ...
    def forward_decoder_base(
        ...
        if want_probs:
            probs = self.get_normalized_probs_base(
                x, log_probs=True, temperature = temperature, sample=None
            )
            probs = probs[:, -1, :]
            return probs, None
    ...
    def get_normalized_probs_base(self, net_output, log_probs, temperature = 1.0, sample=None):
        """Get normalized probabilities (or log probs) from a net's output."""
        # logits = net_output[0].float()
        logits = net_output  # [0].float()
        if log_probs:
    ...
    #     )
    #     return BARTHubInterface(args, task, models[0])
...
@register_model("bart_vf")
class BARTModelVF(BARTModel):
    __jit_unused_properties__ = ["supported_targets"]

    def __init__(self, args, encoder, decoder):
        # TODO: check args, rewrite init method
        super().__init__(args, encoder, decoder)

        # self.classification_heads = nn.ModuleDict()
        self.classification_heads = BARTClassificationHead(
            input_dim=1132, # self.args.encoder_embed_dim,
    ...
    def forward(
        ...
        #         x = head(sentence_representation)
        #         break
        # print('wz test model: ', x.shape,  x[src_tokens.eq(eos), :].shape)
        sentence_representation = x[src_tokens.eq(eos), :].view(
            x.size(0), -1, x.size(-1)
        )[:, -1, :]
        x = self.classification_heads.forward(sentence_representation)
        return x, extra


@register_model_architecture("bart_vf", "bart_large_vf")
