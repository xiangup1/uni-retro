...
class SimpleGenerator(nn.Module):
    ...
    def _generate(self, sample):
        ...
        atom_score, encoder_out, padding_mask = self.model.forward_encoder(src_tokens, reaction_type, pro_macc_fp = pro_macc_fp,tar_spe_token = tar_spe_token, tar_num_token = tar_num_token, )
        atom_score_mask = atom_score.gt(0.1).squeeze(-1)
        atom_score_mask[:,0]= False
        atom_score_mask.masked_fill_(padding_mask, False)
        # print('test atom_score: ', rc_product.shape, atom_score_mask.shape, atom_score_mask)
        # expand to beam size the source latent representations

        atom_score_mask = atom_score_mask.repeat_interleave(self.beam_size, dim=0) # (B x beam_size) x T x C
        rc_product = rc_product.repeat_interleave(self.beam_size, dim=0) # (B x beam_size) x T x C
        ...
        while cur_len < self.max_seq_len:
            ...
            scores, avg_attn_scores = self.model.forward_decoder(generated[:, :cur_len], encoder_out, self.temperature, encoder_padding_mask = src_padding_mask, rc_product = atom_score_mask)  # (batch_size * beam_size, n_tgt_words) 
