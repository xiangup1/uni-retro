...
class CrossMultiheadAttention(nn.Module):
    def __init__(
        ...
        if talking_heads:
            self.post_softmax_talking_heads = nn.Conv2d(num_heads, num_heads, 1, bias = False)
    ...
    def forward(
        ...
        if self.talking_heads:
            attn = attn.view(bsz, -1, tgt_len, src_len).contiguous()
            attn = self.post_softmax_talking_heads(attn)
            attn = attn.view(-1, tgt_len, src_len).contiguous()
            attn = softmax_dropout(attn_weights, self.dropout, self.training, bias=attn_bias)
