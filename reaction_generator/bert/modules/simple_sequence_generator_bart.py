...
class SimpleGeneratorBart(nn.Module):
    def __init__(
        self,
        model,
        dict,
    ...
    def repeat_interleave_dict(self, encoder_out_dict):
        repeat_encoder_out = {}
        for (key, value) in encoder_out_dict.items():
            if len(value) == 0:
                repeat_encoder_out[key] = value
                continue
            elif len(value) == 1:
                if key == 'encoder_out':
                    value[0] = value[0].repeat_interleave(
                    self.beam_size, dim=1)  # (B x beam_size) x T x C 
                else:
                    value[0] = value[0].repeat_interleave(
                    self.beam_size, dim=0)  # (B x beam_size) x T x C  

            repeat_encoder_out[key] = value
        return repeat_encoder_out
    ...
    def _generate(self, sample):
        ...
        if "flag_reaction" in net_input.keys():
            flag_reaction = net_input["flag_reaction"]
        else:
            flag_reaction = None

        encoder_out = self.model.forward_encoder(
            src_tokens = src_tokens, flag_reaction = flag_reaction,)

        encoder_out = self.repeat_interleave_dict(encoder_out)
        if flag_reaction is not None:
            flag_reaction = flag_reaction.repeat_interleave(
                    self.beam_size, dim=0)
        # generated sentences (batch with beam current hypotheses)
        generated = src_tokens.new(
            batch_size * self.beam_size, self.max_seq_len).fill_(self.pad_idx)  # upcoming output
        ...
        beam_scores = encoder_out['encoder_out'][0].transpose(0,1).new(batch_size, self.beam_size).fill_(
            0).to(src_tokens.device)  # 定义scores向量，保存累加的log_probs
        beam_scores[:, 1:] = -1e4  # 需要初始化为-inf
        ...
        while cur_len < self.max_seq_len:
            ...
            scores, extras = self.model.forward_decoder_base(
                encoder_out, generated[:, :cur_len], flag_reaction = flag_reaction,)  # (batch_size * beam_size, n_tgt_words)

            n_tgt_words = scores.size(-1)
            # - scores: (batch_size * beam_size, n_tgt_words)
            _scores = scores + \
                beam_scores.view(batch_size * self.beam_size,
                                 1)  # 累加上以前的scores
            ...
            for sent_id in range(batch_size):
                ...
                if done[sent_id]:
                    next_batch_beam.extend(
                        [(0, self.pad_idx, 0)] * self.beam_size)  # pad the batch
                    continue
                ...
                for idx, value in zip(next_words[sent_id], next_scores[sent_id]):
                    ...
                    if word_id == self.eos_idx or cur_len + 1 == self.max_seq_len:
                        ...
                        next_sent_beam.append(
                            (value, word_id, effective_beam_id))
                    # the beam for next step is full
                    if len(next_sent_beam) == self.beam_size:
                        break
                # update next beam content
                if len(next_sent_beam) == 0:
                    next_sent_beam = [(0, self.pad_idx, 0)] * \
            ...
            # print('wz test generated: ', generated[:, :cur_len])
