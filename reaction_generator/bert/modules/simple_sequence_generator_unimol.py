...
class SimpleGeneratorUnimol(nn.Module):
    def __init__(
        self,
        model,
        dict,
    ...
    def _generate(self, sample):
        ...
        encoder_out, padding_mask, masked_tokens, unimol_classifier_logits, _ = self.model.forward_encoder(
            **sample["net_input"])
        src_padding_mask = padding_mask
        if src_padding_mask is None:
            # src_padding_mask = src_tokens.eq(self.model.padding_idx)
            src_padding_mask = torch.zeros([encoder_out.shape[0], encoder_out.shape[1]]).to(encoder_out.device)
        ...
        while cur_len < self.max_seq_len:
            ...
            scores, avg_attn_scores = self.model.forward_decoder(
                generated[:, :cur_len], encoder_out, self.temperature, encoder_padding_mask=src_padding_mask, train_force_decoding = False)  # (batch_size * beam_size, n_tgt_words)
