...
from .. import utils as customized_utils
from ..data import data_utils
from ..modules.ngram_repeat_block import NGramRepeatBlock
from ..modules.incremental_decoder import IncrementalDecoder
...
class SequenceScoreGeneratorBeamSearch(nn.Module):
    ...
    def _generate(
        ...
        **kwargs
        ...
        if 'flag_reaction' in net_input:
            flag_reaction = net_input['flag_reaction']
        else:
            flag_reaction = None
        ...
        encoder_outs = self.model.forward_encoder(src_tokens, flag_reaction = flag_reaction)
        new_order = torch.arange(bsz).view(-1, 1).repeat(1, beam_size).view(-1)
        new_order = new_order.to(src_tokens.device).long()
        flag_reaction = [flag_reaction]
        encoder_outs, flag_reaction = self.model.reorder_encoder_out(encoder_outs, new_order, flag_reaction)
        ...
        def is_finished(sent, step, unfin_idx, unfinalized_scores=None):
            ...
            if len(finalized[sent]) == beam_size:
                ...
                if self.normalize_scores:
                    #### ttt
                    best_unfinalized_score /= (step+1) ** self.len_penalty
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            if reorder_state is not None:
                ...
                encoder_outs, flag_reaction = self.model.reorder_encoder_out(encoder_outs, reorder_state, flag_reaction)

            lprobs, avg_attn_scores = self.model.forward_decoder(
                tokens[:, :step + 1], encoder_outs, flag_reaction, incremental_states, temperature=self.temperature,
            )
    ...
    def generate_batched_itr(self, data_itr, beam_size=None, cuda=False, timer=None):
        ...
        for sample in data_itr:
            ...
            for i, id in enumerate(s["id"].data):
                # remove padding
                src = customized_utils.strip_pad(input["src_tokens"].data[i, :], self.pad)
                ref = (
                    customized_utils.strip_pad(s["target"].data[i, :], self.pad)
...
class EnsembleModel(nn.Module):
    ...
    @torch.jit.export
    def forward_encoder(self, src_tokens: Dict[str, Tensor], flag_reaction, ):
        if not self.has_encoder():
            return None
        return [model.encoder.forward(src_tokens, flag_reaction=flag_reaction) for model in self.models]
    ...
    def forward_decoder(
        ...
        for i, model in enumerate(self.models):
            if self.has_encoder():
                ...
                flag_reaction_i = flag_reaction[i]
            # decode each model
            if self.has_incremental_states():
                decoder_out = model.decoder.forward(
                    ...
                    flag_reaction = flag_reaction_i,
                )
            else:
                if hasattr(model, "decoder"):
                    # probs, _ = model.forward_decoder(
                    #                 encoder_out, tokens)
                    decoder_out = model.decoder.forward(tokens, encoder_out=encoder_out, flag_reaction = flag_reaction_i,)
                else:
    ...
    def reorder_encoder_out(
        self, encoder_outs: Optional[List[Dict[str, List[Tensor]]]], new_order, flag_reaction
    ):
        """
        ...
        for i, model in enumerate(self.models):
            assert encoder_outs is not None
            reorder_encoder_outs = model.encoder.reorder_encoder_out(encoder_outs[i], new_order, flag_reaction)
            new_outs.append(
                reorder_encoder_outs[0]
            )            
            new_flag_reactions.append(reorder_encoder_outs[1][0])
        return new_outs, new_flag_reactions
...
class SequenceGeneratorWithAlignment(SequenceScoreGeneratorBeamSearch):
    def __init__(
        ...
        if print_alignment == "hard":
            self.extract_alignment = customized_utils.extract_hard_alignment
        elif print_alignment == "soft":
            self.extract_alignment = customized_utils.extract_soft_alignment
