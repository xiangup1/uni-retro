...
from torch import Tensor
from .score_beam_search_generator import EnsembleModel
...
class SimpleGenerator(nn.Module):
    ...
    def _generate(self, sample):
        self.model.eval()
        if use_ensembleModel:
            src_tokens = self.model.single_model.get_src_tokens(sample)
        ...
        if use_ensembleModel:
            ...
            encoder_out = self.model.reorder_encoder_out(encoder_result, new_order)
        ...
        while cur_len < self.max_seq_len:
            if use_ensembleModel:
                if reorder_state is not None:
                    ...
                    encoder_out = self.model.reorder_encoder_out(encoder_out, reorder_state)
