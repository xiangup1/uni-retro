    ...
    def forward(
        ...
        **kwargs
        ...
        with utils.torch_seed(self.args.seed, self.get_num_updates()):
            if self.training:
                num_recycle = int((torch.rand(1)*self.args.recycling).data)
            else:
                num_recycle = self.args.recycling - 1
...
def unimol_confG_architecture(args):
    def base_architecture(args):
        ...
        args.encoder_type = getattr(args, "backbone", "transformer")
    base_architecture(args)

@register_model_architecture("mol_confG", "mol_confG_tri")
def unimol_confG_architecture(args):
    def base_architecture(args):
        args.encoder_layers = getattr(args, "encoder_layers", 15)
        args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
        ...
        args.masked_coord_loss = getattr(args, "masked_coord_loss", 1.0)
        args.masked_dist_loss = getattr(args, "masked_dist_loss", 1.0)
        args.encoder_type = getattr(args, "backbone", "transformer_tri")
    base_architecture(args)
