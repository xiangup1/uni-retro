...
from .transformer_m_layers import (
    AtomFeature,
    MoleculeAttnBias,
    Molecule3DBias,
    AtomTaskHead,
...
class TransformerMEncoder(nn.Module):
    ...
    def forward(
        ...
        if self.training:
            ...
            mask = torch.tensor([mask_dict[i] for i in mask_choice]).to(
                batched_data["pos"]
            )
            mask_2d = mask[:, 0]
            mask_3d = mask[:, 1]
        ...
        if self.molecule_3d_bias is not None and not (batched_data["pos"] == 0).all():
            ...
            x[:, 1:, :] = x[:, 1:, :] + merged_edge_features.type(dtype) * 0.01

        attn_bias = attn_bias.type(dtype)
        ...
        if delta_pos is not None:
            atom_output = self.atom_proc(
                x[1:, :, :], attn_bias[:, :, 1:, 1:], delta_pos
            )
            if mask_3d is not None:
                mask_3d_only = (mask == torch.tensor([0.0, 1.0]).to(mask)[None, :]).all(
                    dim=-1
                )
