...
from .transformer_m_encoder import init_params, TransformerMEncoder

logger = logging.getLogger(__name__)


@register_model("Transformer-M")
class TransformerMModel(BaseUnicoreModel):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default="0.2,0.2,0.6",
            help="probability of {2D+3D, 2D, 3D} mode for joint training",
        )
        parser.add_argument(
            "--add-3d", action="store_true", help="add 3D attention bias"
        )
        parser.add_argument("--no-2d", action="store_true", help="remove 2D encodings")
        parser.add_argument(
            "--num-3d-bias-kernel",
            type=int,
            default=128,
...
class TransformerM(BaseUnicoreModel):
    def __init__(self, args):
        super().__init__()
        self.max_positions = args.max_positions
        
        num_edges_size = 4 if (hasattr(args, 'use_stereoisomerism') and args.use_stereoisomerism is True) else 3
        self.molecule_encoder = TransformerMEncoder(
            num_atoms=512 * 9,
            num_in_degree=512,
            num_out_degree=512,
            num_edges=512 * num_edges_size,
            num_spatial=512,
            num_edge_dis=128,
        ...
        self.embed_out = nn.Linear(args.encoder_embed_dim, 1, bias=False)
