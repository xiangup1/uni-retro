...
def align_to(src_mol, ref_mol):
    ...
    return src_atoms, src_pos, ref_pos
...
def process_one(index):
    ...
    # try rdkit
    try:
        init_mol = rdkit_3d_gen(ori_smi)
        init_mol = Chem.RemoveHs(init_mol)
        mols.append(init_mol)
    except:
        init_mol = None
    ...
    try:
        ...
        init_mol = None
        try:
            init_mol = obabel_3d_gen_by_sdf(ori_smi, index)
            init_mol = Chem.RemoveHs(init_mol)
            mols.append(init_mol)
        except:
            init_mol = None
    ...
    if len(mols) == 0:
        try:
            init_mol = rdkit_gen(ori_smi)
            init_mol = Chem.RemoveHs(init_mol)
            mols.append(init_mol)
            print("rollback", index, ori_smi)
        except:
            print("fail", index, ori_smi)
            return key, None

    if len(mols) == 1:
        atoms, init_pos = get_info(mols[0])
        init_pos_list = [init_pos]
        label_pos = init_pos
    else:
        try:
            atoms, src_pos, label_pos = align_to(mols[0], mols[1])
            init_pos_list = [label_pos, src_pos]
        except:
            atoms, init_pos = get_info(mols[1])
            init_pos_list = [init_pos]
            label_pos = init_pos
    if len(atoms) <= 0:
        print(index, ori_smi)
        return key, None
    node_attr, edge_index, edge_attr = get_graph(mols[-1])
...
os.system("rm -f {}.lmdb".format(split_key))
...
with Pool(96) as pool:
    for ret in tqdm(pool.imap(process_one, valid_index), total=len(valid_index)):
        key, val = ret
